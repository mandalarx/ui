...
function Example() {
  ...
  return <div className="mx-auto w-full max-w-xl space-y-5"><UI.Form {...form}><form onSubmit={form.handleSubmit(() => undefined)} className="space-y-4"><UI.FormField control={form.control} name="name" rules={{ required: "Please enter a collection name." }} render={({ field }) => <UI.FormItem><UI.FormLabel>Collection name</UI.FormLabel><UI.FormControl><Input {...field} placeholder="Azure Glass" /></UI.FormControl><UI.FormDescription>Leave empty and submit to inspect validation.</UI.FormDescription><UI.FormMessage /></UI.FormItem>} /><Button type="submit">Validate</Button></form></UI.Form></div>
}
