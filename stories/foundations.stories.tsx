...
const palette = [
  ...
  ["Frost", "bg-card", "text-card-foreground"],
  ...
  return <section className="space-y-5" aria-labelledby={titleId}>
    ...
    <div className="azure-surface-stage grid gap-6 rounded-[28px] border p-5 sm:p-8 md:grid-cols-2">
      {(["solid", "glass"] as const).map(surface => <Card key={surface} data-surface={surface} data-testid={`surface-${surface}`}>
        <CardHeader><div className="mb-3 flex items-center justify-between"><Badge variant="outline" className="font-mono text-[10px] uppercase tracking-wider">{surface === "solid" ? "01 / Solid" : "02 / Glass"}</Badge><Layers className="size-4 text-primary" /></div><CardTitle className="text-xl">{surface === "solid" ? "A steady foundation." : "A lighter perspective."}</CardTitle></CardHeader>
    ...
    <div className="flex flex-wrap gap-x-6 gap-y-2 font-mono text-[10px] uppercase tracking-wider text-muted-foreground"><span>Controls / 12px</span><span>Menus / 16px</span><span>Cards / 20px</span><span>Dialogs / 24px</span></div>
  </section>
}
